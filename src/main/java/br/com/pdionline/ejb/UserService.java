...
public class UserService {
	...
	public String create(User user) {
		...
		if (this.checkUserExist(user) && user.getId() == null) {
	...
	/**
	 * @return true if user already exist or false if user not exists
	 */
	private boolean checkUserExist(User user) {
		boolean alreadyExists = false;

		Query query = em.createQuery("select count(o.id) from User o where o.email = :email and o.name = :name ");
		query.setParameter("email", user.getEmail());
		query.setParameter("name", user.getName());

		Object object = query.getSingleResult();

		alreadyExists = (Long) object > 0;
